...
syms l m b g u1 u2 u3 u4

% Torques in the direction of phi, theta, psi
tau_beta = [l*(-u2+u4); l*(-u1+u3); b*(-u1+u2-u3+u4)];
...
dot_alpha =[ state(10) + state(12)*cos(state(4))*tan(state(5)) + state(11)*sin(state(4))*tan(state(5));
                                          state(11)*cos(state(4)) - state(12)*sin(state(4));
                              (state(12)*cos(state(4)) + state(11)*sin(state(4)))/cos(state(5))];
 
dot_v =R*([0; 0; u1+u2+u3+u4])/m - [0; 0; g];
dot_w = I\( tau_beta - cross( state(10:12,1) , I * state(10:12,1) ) );
...
f = subs(f, [Ixx Iyy Izz l m b g], ...
    [IxxVal IyyVal IzzVal lVal mVal bVal gVal]);
f = simplify(f);
